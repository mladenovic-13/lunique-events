...
interface EventOrganizationInputProps {
  value: string | null;
  onChange: (value: string) => void;
}

export function EventOrganizationInput({
  onChange,
}: EventOrganizationInputProps) {
  ...
  return (
    <div>
      <Select value={organizationId} onValueChange={updateOrganizationId}>
        <SelectTrigger className="h-8 justify-between border-muted-foreground/10 bg-muted data-[state=open]:bg-muted-foreground/50 md:max-w-full">
